...
class LaterPeriodViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
    
    static var cellNumber: Int = 0
    static var laterPeriodClassArray: [Class] = []
    ...
    override func viewWillAppear(_ animated: Bool) {
        // 取得
        if let data = UserDefaults.standard.object(forKey: "LaterPeriodTimeTable") as? Data, let array = NSKeyedUnarchiver.unarchiveObject(with: data) as? [Class] {
            LaterPeriodViewController.laterPeriodClassArray = array
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        LaterPeriodViewController.cellNumber = indexPath.row
        performSegue(withIdentifier: "Modal2", sender: self)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let data = LaterPeriodViewController.laterPeriodClassArray.first(where: { $0.cellNumber == indexPath.row }) {
