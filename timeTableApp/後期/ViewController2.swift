...
class ViewController2: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //        取得
        if let data = UserDefaults.standard.object(forKey: "TimeTable2") as? Data, let array = NSKeyedUnarchiver.unarchiveObject(with: data) as? [Class] {
            ViewController2.testArray2 = array
//            print("array:\(array)")
        } else {
            print("Error")
        }
        print("testArray2:\(ViewController2.testArray2)")
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        ViewController2.cellNumber = indexPath.row
        
        if ViewController2.cellNumber == indexPath.row {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let data = ViewController2.testArray2.first(where: { $0.cellNumber == indexPath.row }) {
